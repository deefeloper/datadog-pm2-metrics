...
DataDogPm2Metrics.prototype.collectPm2Metrics = function (cb) {
  pm2.list(function (err, result) {console.log(result);
    if (err) {
      return cb(err);
    }
...
DataDogPm2Metrics.prototype.sendPm2MetricsToDatadog = function (pm2Metrics, cb) {
  ...
  pm2Metrics.forEach(function (pm2InstanceMetrics) {
    ...
    self.statsDClient.check('pm2_process.running', pm2InstanceMetrics.metrics.running ? self.statsDClient.CHECKS.OK : self.statsDClient.CHECKS.CRITICAL, tags);

    self.statsDClient.gauge('pm2_process.cpu', pm2InstanceMetrics.metrics.cpu, tags);
    self.statsDClient.gauge('pm2_process.memory', pm2InstanceMetrics.metrics.memory, tags);
  });
